...
fun main() {
    ...
    fun part1(input: List<String>): Int {
        val blueprints = input.map(Blueprint.Companion::parse)
    ...
    fun part2(input: List<String>): Int {
        val blueprints = input.map(Blueprint.Companion::parse).take(3)
        return blueprints
            .map { blueprint -> blueprint.simulate(32) }
            .reduceRight { left, right -> left * right }
    }

    val testInput = readInput("Year2022/Day19_test")
...
private class Blueprint(
    ...
    companion object {
        fun parse(input: String): Blueprint {
            val id = input.substringAfter("Year2022.Blueprint ").substringBefore(":").toInt()
