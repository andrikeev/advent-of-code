...
fun main() {
    ...
    val testInput = readInput("Year2022/Day13_test")
...
private sealed interface DataValue : Comparable<DataValue> {
    ...
    override operator fun compareTo(other: DataValue): Int {
        when {
            this is IntValue && other is IntValue -> {
                return this.value.compareTo(other.value)
            }

            this is ListValue && other is ListValue -> {
                repeat(minOf(this.value.size, other.value.size)) { i ->
                    val comp = this.value[i].compareTo(other.value[i])
                    if (comp != 0) {
                        return comp
                    }
                }
                ...
                this is IntValue -> return this.toList().compareTo(other)
                other is IntValue -> return this.compareTo(other.toList())
            }
        }
